...
usage () {
    ...
    ARGS must be in specified order, aren't optional, with no flag.
    --ARGS[0] is the directory you wish to backup aka Drupal Root:
       Example /var/www/vhosts/someclient.com/www
...
quick_archive () {
    ...
    # This is has portable issue, via Client authentication is handled by PAM
    mysqldump -u $MYSQL_USER ${ARGS[2]} > ${ARGS[2]}_$TODAY.sql
